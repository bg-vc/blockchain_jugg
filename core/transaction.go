...
func (tx *Transaction) TrimmedCopy() Transaction {
	...
	for _, vout := range tx.Vout {
		outputs = append(outputs, TXOutput{vout.Index, vout.Value, vout.PubKeyHash})
	}
...
func NewCoinbaseTX(to, data string) *Transaction {
	if data == "" {
		...
		fmt.Printf("NewCoinbaseTX data: %x \n", data)
	}
...
func NewUTXOTransaction(Wallet *wallet.Wallet, to string, amount int, UTXOSet *UTXOSet) *Transaction {
	...
	outputs = append(outputs, *NewTXOutput(0, amount, to))
	if acc > amount {
		outputs = append(outputs, *NewTXOutput(1, acc-amount, from)) // a change
	}
